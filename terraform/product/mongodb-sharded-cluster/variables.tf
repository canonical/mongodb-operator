...
variable "config-server" {
  description = "Config server orchestrator app definition"
  type = object({
    app_name          = string
    model             = string
...
variable "sharded" {
  description = "Shard orchestrator app definition"
  type = object({
    app_name          = string
    model             = string
...
variable "mongos" {
  description = "Configuration for the mongos"
  type = object({
    channel     = optional(string, "6/stable")
    revision    = optional(string, 23)
    base        = optional(string, "ubuntu@22.04")
    constraints = optional(string, "arch=amd64")
    machines    = optional(list(string), [])
    config      = optional(map(string), {})
