...
resource "juju_offer" "mongodb_client_offer" {
  for_each = var.data_integrator.model_uuid != var.mongodb.model_uuid ? { "offered" = true } : {}
...
resource "juju_offer" "tls_provider_offer" {
  for_each = local.enable_tls && var.self_signed_certificates.model_uuid != var.mongodb.model_uuid ? { "offered" = true } : {}
...
resource "juju_offer" "s3_integrator_offer" {
  for_each = var.s3_integrator.model_uuid != var.mongodb.model_uuid ? { "offered" = true } : {}
