...
class MongoDBBackups(Object):
    ...
    def _on_s3_credential_changed(self, event: CredentialsChangedEvent):
        ...
        if "db_initialised" not in self.charm.app_peer_data:
            self._defer_action_with_info_log(
                event, action, "Cannot set PBM configurations, MongoDB has not yet started."
            )
            return
        ...
        if not pbm_snap.present:
            self._defer_action_with_info_log(
                event, action, "Cannot set PBM configurations, PBM snap is not yet installed."
            )
            return
        ...
        try:
            ...
            # CredentialsChangedEvent can't be failed, so just log error
    ...
    def _on_create_backup_action(self, event) -> None:
        ...
        if self.model.get_relation(S3_RELATION) is None:
            self._fail_action_with_error_log(
                event, action, "Relation with s3-integrator charm missing, cannot create backup."
            )
            return
    ...
    def _on_list_backups_action(self, event) -> None:
        ...
        if self.model.get_relation(S3_RELATION) is None:
            self._fail_action_with_error_log(
                event, action, "Relation with s3-integrator charm missing, cannot list backups."
            )
            return
