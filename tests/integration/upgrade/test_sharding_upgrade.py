...
async def test_upgrade(
    ...
    # scale up shard2 to three units since it only has one relplica and when it restarts due to an
    #  upgrade it could lose some writes.
    await ops_test.model.applications[SHARD_TWO_APP_NAME].add_unit(count=2)
    await ops_test.model.wait_for_idle(
        apps=CLUSTER_COMPONENTS, idle_period=20, timeout=TIMEOUT, raise_on_blocked=False
    )
