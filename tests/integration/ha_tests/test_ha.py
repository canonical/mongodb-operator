...
from . import helpers as ha_helpers  # TODO unify import with helpers
...
async def test_build_and_deploy(ops_test: OpsTest) -> None:
    ...
    if user_app_name:
        return check_or_scale_app(ops_test, user_app_name, required_units)
...
async def test_storage_re_use(ops_test, continuous_writes):
    ...
    if ha_helpers.storage_type(ops_test, app_name) == "rootfs":
        pytest.skip(
    ...
    unit_storage_id = ha_helpers.storage_id(ops_test, unit.name)
    ...
    new_unit = await ha_helpers.add_unit_with_storage(ops_test, app_name, unit_storage_id)

    assert await ha_helpers.reused_storage(
        ops_test, new_unit.public_address, removal_time
    ), "attached storage not properly reused by MongoDB."

    # verify that the no writes were skipped
    total_expected_writes = await ha_helpers.stop_continous_writes(ops_test)
    actual_writes = await ha_helpers.count_writes(ops_test)
...
async def test_add_units(ops_test: OpsTest, continuous_writes) -> None:
    ...
    member_ips = await ha_helpers.fetch_replica_set_members(ip_addresses, ops_test)
...
async def test_scale_down_capablities(ops_test: OpsTest, continuous_writes) -> None:
    ...
    try:
        primary = await ha_helpers.replica_set_primary(ip_addresses, ops_test)
    except RetryError:
        primary = None
...
async def test_replication_across_members(ops_test: OpsTest, continuous_writes) -> None:
    ...
    await ha_helpers.insert_focal_to_cluster(ops_test)
    app_name = await get_app_name(ops_test)
    ip_addresses = [unit.public_address for unit in ops_test.model.applications[app_name].units]
    primary = await ha_helpers.replica_set_primary(ip_addresses, ops_test)
    ...
    for secondary in secondaries:
        client = MongoClient(
            ha_helpers.unit_uri(secondary, password, app_name), directConnection=True
        )
...
async def test_unique_cluster_dbs(ops_test: OpsTest, continuous_writes) -> None:
    ...
    password = await ha_helpers.get_password(ops_test, app=ANOTHER_DATABASE_APP_NAME)
    client = ha_helpers.replica_set_client(ip_addresses, password, app=ANOTHER_DATABASE_APP_NAME)
    db = client["new-db"]
    test_collection = db["test_ubuntu_collection"]
    test_collection.insert_one({"release_name": "Jammy Jelly", "version": 22.04, "LTS": False})
    client.close()

    cluster_1_entries = await ha_helpers.retrieve_entries(
        ops_test,
    ...
    cluster_2_entries = await ha_helpers.retrieve_entries(
        ops_test,
        app=ha_helpers.APP_NAME,
        db_name="new-db",
        collection_name="test_ubuntu_collection",
        query_field="release_name",
    )
...
async def test_replication_member_scaling(ops_test: OpsTest, continuous_writes) -> None:
    ...
    client = MongoClient(ha_helpers.unit_uri(new_member_ip, password, app), directConnection=True)
...
async def test_kill_db_process(ops_test, continuous_writes):
    ...
    await ha_helpers.kill_unit_process(ops_test, primary.name, kill_code="SIGKILL")

    # verify new writes are continuing by counting the number of writes before and after a 5 second
    # wait
    writes = await ha_helpers.count_writes(ops_test)
    time.sleep(5)
    more_writes = await ha_helpers.count_writes(ops_test)
    ...
    assert await ha_helpers.mongod_ready(ops_test, primary.public_address)

    # verify that a new primary gets elected (ie old primary is secondary)
    new_primary = await ha_helpers.replica_set_primary(ip_addresses, ops_test)
    ...
    assert await ha_helpers.secondary_up_to_date(
...
async def test_freeze_db_process(ops_test, continuous_writes):
    ...
    ip_addresses = [unit.public_address for unit in ops_test.model.applications[app].units]
    primary = await ha_helpers.replica_set_primary(ip_addresses, ops_test)
    await ha_helpers.kill_unit_process(ops_test, primary.name, kill_code="SIGSTOP")
    ...
    await ha_helpers.kill_unit_process(ops_test, primary.name, kill_code="SIGCONT")
    ...
    assert (
        await ha_helpers.count_primaries(ops_test) == 1
...
async def test_restart_db_process(ops_test, continuous_writes, change_logging):
    ...
    app = await ha_helpers.app_name(ops_test)
    ip_addresses = [unit.public_address for unit in ops_test.model.applications[app].units]
    old_primary = await ha_helpers.replica_set_primary(ip_addresses, ops_test)

    # send SIGTERM, we expect `systemd` to restart the process
    sig_term_time = time.time()
    await ha_helpers.kill_unit_process(ops_test, old_primary.name, kill_code="SIGTERM")
    ...
    # verify that db service got restarted and is ready
    assert await ha_helpers.mongod_ready(ops_test, old_primary.public_address)
    ...
    try:
        for attempt in Retrying(stop=stop_after_delay(30), wait=wait_fixed(3)):
            with attempt:
                assert await ha_helpers.db_step_down(
...
async def test_full_cluster_crash(ops_test: OpsTest, continuous_writes, reset_restart_delay):
    ...
    for unit in ops_test.model.applications[app].units:
        await ha_helpers.update_restart_delay(ops_test, unit, RESTART_DELAY)

    # kill all units "simultaneously"
    await asyncio.gather(
        *[
            ha_helpers.kill_unit_process(ops_test, unit.name, kill_code="SIGKILL")
            for unit in ops_test.model.applications[app].units
        ]
    )
    ...
    assert await ha_helpers.all_db_processes_down(ops_test), "Not all units down at the same time."
    ...
    for unit in ops_test.model.applications[app].units:
        assert await ha_helpers.mongod_ready(
    ...
    await ha_helpers.verify_replica_set_configuration(ops_test)
...
async def test_full_cluster_restart(ops_test: OpsTest, continuous_writes, reset_restart_delay):
    ...
    await asyncio.gather(
        *[
            ha_helpers.kill_unit_process(ops_test, unit.name, kill_code="SIGTERM")
            for unit in ops_test.model.applications[app].units
        ]
    )
...
async def test_network_cut(ops_test, continuous_writes):
    ...
    primary_hostname = await ha_helpers.unit_hostname(ops_test, primary.name)
    primary_unit_ip = await ha_helpers.get_unit_ip(ops_test, primary.name)
    ...
    for unit in set(all_units) - {primary}:
        ...
        assert not ha_helpers.is_machine_reachable_from(
            hostname, primary_hostname
        ), "unit is reachable from peer"

    # verify machine is not reachable from controller
    controller = await ha_helpers.get_controller_machine(ops_test)
    assert not ha_helpers.is_machine_reachable_from(
    ...
    # verify new writes are continuing by counting the number of writes before and after a 5 second
    # wait
    writes = await ha_helpers.count_writes(ops_test, down_unit=primary.name)
    time.sleep(5)
    more_writes = await ha_helpers.count_writes(ops_test, down_unit=primary.name)
    assert more_writes > writes, "writes not continuing to DB"

    # verify that a new primary got elected
    new_primary = await ha_helpers.replica_set_primary(
        ip_addresses, ops_test, down_unit=primary.name
    )
    assert new_primary.name != primary.name

    # verify that no writes to the db were missed
    total_expected_writes = await ha_helpers.stop_continous_writes(
        ops_test, down_unit=primary.name
    )
    actual_writes = await ha_helpers.count_writes(ops_test, down_unit=primary.name)
    assert total_expected_writes["number"] == actual_writes, "writes to the db were missed."

    # restore network connectivity to old primary
    ha_helpers.restore_network_for_unit(primary_hostname)

    # wait until network is reestablished for the unit
    ha_helpers.wait_network_restore(model_name, primary_hostname, primary_unit_ip)
    ...
    new_ip = ha_helpers.instance_ip(model_name, primary_hostname)
    assert await ha_helpers.mongod_ready(
        ops_test,
        new_ip,
...
async def scale_and_verify(ops_test: OpsTest, count: int, remove_leader: bool = False):
    ...
    ip_addresses = [unit.public_address for unit in ops_test.model.applications[app].units]
    primary = await ha_helpers.replica_set_primary(ip_addresses, ops_test, app=app)
...
async def reset_restart_delay(ops_test: OpsTest):
    ...
    app = await ha_helpers.app_name(ops_test)
    for unit in ops_test.model.applications[app].units:
        await ha_helpers.update_restart_delay(ops_test, unit, ORIGINAL_RESTART_DELAY)
...
async def change_logging(ops_test: OpsTest):
    ...
    for unit in ops_test.model.applications[app].units:
        ...
        await ha_helpers.stop_mongod(ops_test, unit)
        await ha_helpers.update_service_logging(ops_test, unit, logging=True)
        await ha_helpers.start_mongod(ops_test, unit)
    ...
    for unit in ops_test.model.applications[app].units:
        # must restart unit to ensure that changes to logging are made
        await ha_helpers.stop_mongod(ops_test, unit)
        await ha_helpers.update_service_logging(ops_test, unit, logging=False)
        await ha_helpers.start_mongod(ops_test, unit)
        ...
        rm_cmd = f"exec --unit {unit.name} rm {ha_helpers.MONGODB_LOG_PATH}"
